...
def directional_vgm(rawvgm, directions, h_width=None, h_bins=15, bandwidth=None,
    ...
    h_width, h_bins : float / int
        Bin width, or number of bins if ``h_width`` is None.
    ...
    DataFrame
        ...
        (mean distance in the bin), ``variogram`` and ``count``.
    """
    ...
    for j in range(n_dir):
        ...
        proj = dh @ u                              # signed projection on axis
        cos_ang = np.abs(proj) / safe_h            # |cos(angle to axis)|
        mask = cos_ang >= cos_tol
        if bandwidth is not None:
            perp = np.sqrt(np.maximum(h ** 2 - proj ** 2, 0.0))
            mask &= perp <= bandwidth
        if not np.any(mask):
            continue
        hh, gg = h[mask], g[mask]
        bins = (hh // h_width).astype(int)
...
def plot_vgm_map3d(rawvgm, x="d0", y="d1", z="d2", dist="distance",
                   ...
                   angleh="angle_h", anglev="angle_v", angle_aniso=None,
                   vgm="variogram", cmap="viridis_r", vmin=None, vmax=None,
                   slab_frac=0.75, bandwidth_factor=2.0,
                   kws_cbar=None, title="3D Variogram Map"):
    """3D variogram map shown as two orthogonal fences.

    A *horizontal* slice (lags with ``|d2|`` within ``slab_frac * dz``) is drawn
    in the z=0 plane, and a *vertical* slice (lags aligned with the major
    azimuth) is drawn as an upright fence.  Together they visualise horizontal
    and vertical continuity in a single 3D view.

    ``angle_aniso`` may be a scalar azimuth or ``(azimuth, dip)``; if None it is
    estimated with :func:`estimate_aniso_angle`.
    """
    kws_cbar = kws_cbar or {"label": "Variogram", "pad": 0.05}
    if dx is None:
        dx = abs(np.quantile(rawvgm[x], 0.9) / 20)
    if dy is None:
        dy = dx
    if dz is None:
        dz = abs(np.quantile(rawvgm[z], 0.9) / 20)

    df = rawvgm[rawvgm[dist] <= cutoff].copy() if cutoff is not None else rawvgm.copy()
    if vmin is None:
        vmin = df[vgm].quantile(0.05)
    if vmax is None:
        vmax = df[vgm].quantile(0.95)
    norm = Normalize(vmin, vmax)
    cmap_obj = plt.get_cmap(cmap)

    if angle_aniso is None:
        angle_aniso = estimate_aniso_angle(df, x, y, dist, vgm, dim3d=False)[0][0]
    azimuth = angle_aniso[0] if np.ndim(angle_aniso) else float(angle_aniso)

    if ax is None:
        fig = plt.figure(figsize=(9, 8))
        ax = fig.add_subplot(111, projection="3d")

    # --- horizontal fence: lags near the z=0 plane -------------------------
    horiz = df[(df[z].abs() <= dz * slab_frac) & ((df[x] != 0) | (df[y] != 0))]
    if len(horiz):
        xx, yy, vv = _grid_average(horiz, x, y, dx, dy, vgm, mirror=True)
        zz = np.zeros_like(xx)
        ax.plot_surface(xx, yy, zz, rstride=1, cstride=1,
                        facecolors=cmap_obj(norm(vv)), shade=False)

    # --- vertical fence: lags aligned with the major azimuth ---------------
    vert = df[filter_vgm(df[x], df[y], df[angleh], azimuth,
                         bandwidth=dx * bandwidth_factor)].copy()
    if len(vert):
        # signed radial coordinate along the azimuth direction
        ux, uy = np.sin(np.deg2rad(azimuth)), np.cos(np.deg2rad(azimuth))
        vert["r"] = vert[x] * ux + vert[y] * uy
        dr = math.hypot(dx, dy)
        rr, zz, vv = _grid_average(vert, "r", z, dr, dz, vgm, mirror=True)
        xx = rr * ux
        yy = rr * uy
        ax.plot_surface(xx, yy, zz, rstride=1, cstride=1,
                        facecolors=cmap_obj(norm(vv)), shade=False)
    ...
    ax.set_zlabel("Z lag")
    if title:
        ax.set_title(f"{title}\nAzimuth of maximum continuity: {azimuth:.1f}$^o$",
                     fontsize="large")
    return ax
